...
if __name__ == '__main__':
    ...
    logger.add('logs/logs_{time}.log', level='DEBUG', format="{time} {level} {message}", rotation="06:00", compression="zip")
    logger.debug('Error')
    ...
    def get_quantity(message: types.Message) -> None:
        if message.text == '/reset':
            ...
            bot.send_message(chat_id=message.chat.id, text=f"Введите цифрами, какое количество отелей будем показывать (максимум {config.quantity_max_hotel})?")
            return
    ...
    def get_quantity_photos(message: types.Message) -> None:
        if message.text == '/reset':
            ...
            bot.send_message(chat_id=message.chat.id, text=f"Введите цифрами, количество фотографий для каждого отеля (максимум {config.quantity_max_photo})?")
            return
