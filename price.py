...
@logger.catch
def req(url: str, headers: str = "", params=None) -> Union[bool, Response]:
    """Функция для проверки существования фото по URL"""
    if params is None:
        params = {}
    try:
...
def request_city(city: str) -> Union[bool, str]:
    ...
    try:
        request = req(url=url, headers=config.headers, params=querystring)
...
def request_photo(id_hotel: str) -> Union[None, list]:
    ...
    photos = []
    try:
        response = req(url, headers=config.headers, params=querystring)
